...
const answerModel = require("../models/ansModel")
...
const allQueAns = async (req, res) => {
    try {
        ...
        const questionData = await questionModel.find({ isDeleted: false,tag:tag1 }).sort({createdAt:sort}).select({ createdAt: 0, updatedAt: 0, askedBy: 0, isDeleted: 0 });
